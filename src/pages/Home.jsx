              ...
              >
                <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                  <div className={`h-32 bg-gradient-to-br ${test.color} flex items-center justify-center`}>
                    <span className="text-6xl">{test.icon === 'Target' ? '🎯' : test.icon === 'Lightbulb' ? '💡' : test.icon === 'Brain' ? '🧠' : test.icon === 'Layout' ? '📋' : test.icon === 'Users' ? '👥' : test.icon === 'Heart' ? '❤️' : test.icon === 'Crown' ? '👑' : test.icon === 'Scale' ? '⚖️' : test.icon === 'Shield' ? '🛡️' : test.icon === 'Zap' ? '⚡' : test.icon === 'MessageCircle' ? '💬' : test.icon === 'Eye' ? '👁️' : test.icon === 'DollarSign' ? '💵' : test.icon === 'Sun' ? '☀️' : test.icon === 'AlertTriangle' ? '⚠️' : test.icon === 'BookOpen' ? '📖' : '❤️'}</span>
                  </div>
                  <div className="p-6">
